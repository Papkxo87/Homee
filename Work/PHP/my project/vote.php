...
if (file_exists("$id.txt")) {
  ...
  if ($vote) {
    ...
    //записываем ip и дату голосования
    $ip_add = json_decode(file_get_contents("ip1.json"));
    $ip_add[] = ["ip" => $ip, "date" => date("d.n.Y")];
...
/*function saveIp($message, $name)
{
    $data = json_decode(file_get_contents("ip1.json"));
    $data[] = ["ip" => $ip, "date" => date("d.n.Y")];
    file_put_contents("ip1.json", json_encode($data, JSON_UNESCAPED_UNICODE));
}*/